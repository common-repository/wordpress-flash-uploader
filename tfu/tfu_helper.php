...
function return_kbytes($val)
{
    ...
    $val = (int) $val;   //2017-02-09 Type casting bugfix
    switch ($last) {
...
function is_text_file($image)
{
    $image = strtolower ($image);
    return preg_match('/.*\.(csv)$/', $image) ||
           preg_match('/.*\.(txt)$/', $image) ||
           preg_match('/.*\.(log)$/', $image) ;
}
...
';
    ...
    echo '<table><tr><td>';
    echo '<tr><td width="400">TFU version:</td><td width="250">3.2.2&nbsp;';
...
function tfu_download($file, $enable_file_download) {
    ...
    while ($content = fread($fp, 2048)) {   
      if ($start && 0 === strncmp($content, $bom, 3)) {
        $content = substr($content, 3);
        $is_bom = true;
        tfu_debug('BOM detected and removed.');
      }
...
global $fix_utf8, $use_smtp, $smtp_host, $smtp_port, $smtp_user, $smtp_password, $smtp_secure;

if (strnatcmp(phpversion(),'5.1.0') >= 0) { // new version - used for php > 5.1 
    if (file_exists('class.phpmailer.php')) {
    require_once('class.phpmailer.php');
       require_once('class.smtp.php');
    } else {
       // we are in Wordpress - but ABSPATH and WPINC are not know here as this code does not run directly in WP. 
       $mailer_path = '../../../../' . $_SESSION["WPINC"] . '/class-phpmailer.php'; 
       if (file_exists($mailer_path)) { 
           require_once($mailer_path);
           require_once('../../../../' . $_SESSION["WPINC"] . '/class-smtp.php');
       } else {
           tfu_debug("class-phpmailer.php is not at the default location. Please adopt the path in tfu_helper.php -> tfu_mail");
       }
    }
    ...
    if ($use_smtp) {
      ...
      $mail->SMTPAuth   = $smtp_user != "";           // enable SMTP authentication
...
function getMaximumChunkSize()
{
    $upload_max = return_kbytes(ini_get('upload_max_filesize'));
    $post_max = return_kbytes(ini_get('post_max_size'));
    $maxsize = $upload_max < $post_max ? $upload_max : $post_max;
    
    // Max chunk size is 16 mb
    if ($maxsize > 16384) {
        return '16mb';   
    } 
    return (round($maxsize / 1024)) . "mb";
}
